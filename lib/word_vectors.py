...
def obj_edge_vectors(names, wv_type='glove.6B', wv_dir='', wv_dim=300, word_vectors=None, avg_words=False):
    if word_vectors is None:
        wv_dict, wv_arr, wv_size = load_word_vectors(wv_dir, wv_type, wv_dim)
    else:
        wv_dict, wv_arr, wv_size = word_vectors

    vectors = torch.Tensor(len(names), wv_dim)
    vectors.normal_(0,1)

    for i, token_ in enumerate(names):
        token = token_.lower()
        wv_index = wv_dict.get(token, None)
        if wv_index is not None:
            vectors[i] = wv_arr[wv_index]
        elif avg_words and len(token.split(' ')) > 1:
            words, vectors_ = [], []
            for word in token.split(' '):
                wv_index = wv_dict.get(word, None)
                if wv_index is not None:
                    vectors_.append(wv_arr[wv_index].view(1, -1))
                    words.append(word)
                else:
                    print('{} not found'.format(word))
            if len(words) > 0:
                # print("{} -> {} ".format(token, words))
                vectors[i] = torch.cat(vectors_, dim=0).mean(0)
            else:
            ...
            if token.find('background') < 0:
                print("{} -> {} ".format(token, lw_token))
            wv_index = wv_dict.get(lw_token, None)
            if wv_index is not None:
                vectors[i] = wv_arr[wv_index]
            elif token.find('background') < 0:
                print("fail on {}".format(token))

    return vectors, (wv_dict, wv_arr, wv_size)
