"""
Code borrowed from from https://github.com/google/sg2im/blob/master/sg2im/crn.py
"""
...
class RefinementModule(nn.Module):
    ...
    def forward(self, layout, feats):
        ...
        if HH > H:
            ...
            layout = F.adaptive_avg_pool2d(layout, output_size=H)
...
class RefinementNetwork(nn.Module):
    ...
    def forward(self, layout):
        ...
        for i, mod in enumerate(self.refinement_modules):
            if i == len(self.refinement_modules) - 1:
                feats = F.upsample(feats, size=(H,W), mode='nearest')
            else:
